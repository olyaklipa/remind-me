...
public interface SubjectRepository extends JpaRepository<Subject, Long> {
    ...
    Optional<Subject> findSubjectByUserAndId(Long userId, Long id);

    @Query("SELECT s FROM Subject s WHERE s.user.id = :userId AND s.name = :name")
    Optional<Subject> findSubjectByUserAndName(Long userId, String name);

    @Query("SELECT s FROM Subject s WHERE s.user.id = :userId")
    List<Subject> findSubjectsByUser(Long userId);
